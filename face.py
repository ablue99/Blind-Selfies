...
def find_face(frame, bound_rect, no_face_flag):
	...
	if all_face_coords != ():
		...
		# determine right/ left orientation of face
...
while repeat:
	...
	# set quad after it recieves a command
	...
	while True:
		...
		(successful, no_face_flag) = find_face(frame, bound_rect, no_face_flag) #looks for face, gives instructions
		...
		if successful: # gives a countdown then reads and saves the current frame
			...
			while True:
				...
				try:
					y_n_reply = r.recognize_google(audio2);
